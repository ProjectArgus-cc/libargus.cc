...
def build(row, testing=False, sanitizer='', media=False):
    ...
    run(['cmake', '-B', 'build', '-G', 'Ninja', '-DCMAKE_BUILD_TYPE=Release', '-DCMAKE_EXPORT_COMPILE_COMMANDS=ON', '-DARGUS_PORTABLE=ON',
         '-DARGUS_TESTING=' + ('ON' if testing else 'OFF'), '-DARGUS_SANITIZER=' + sanitizer,
    ...
    if not sanitizer:
        ...
        if backend == 'cuda':
            ...
            if 'V12.4.127' not in toolkit: raise ValueError('CUDA compiler differs from toolkit 12.4.1')
        elif backend == 'rocm':
